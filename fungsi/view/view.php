...
class view
{
    ...
    public function barang_cari($cari)
    {
        ...
        from barang inner join kategori on barang.id_kategori = kategori.id_kategori
        inner join satuan on barang.id_satuan = satuan.id_satuan
        where id_barang like '%$cari%' or nama_barang like '%$cari%' like '%$cari%'";
        $row = $this-> db -> prepare($sql);
        $row -> execute();
        $hasil = $row -> fetchAll();
        return $hasil;
    ...
    public function hari_barang_jual($hari)
    {
        $ex = explode('-', $hari);
        $monthNum  = $ex[1];
        $monthName = date('F', mktime(0, 0, 0, $monthNum, 10));
    ...
    {
        $sql ="SELECT _temp_penjualan.* , barang.id_barang, barang.nama_barang, barang.harga_jual, barang.harga_beli, member.id_member,
